...
- [ ] Compte Mailtrap créé et email confirmé
- [ ] Identifiants SMTP récupérés depuis le dashboard
- [ ] `symfony/mailer` installé dans le projet
- [ ] `MAILER_DSN` configuré dans `.env` ou `.env.local`
- [ ] Cache Symfony vidé
- [ ] Route de test créée dans un contrôleur
- [ ] Email visible dans la boîte Mailtrap

---

> 💡 **Astuce** : Mailtrap propose aussi une intégration directe avec Symfony via le bridge `mailtrap/symfony-mailer`. Pour le TP, la configuration SMTP classique est suffisante.
